...
class Plug extends EventEmitter {
  ...
  set sysInfo (sysInfo) {
    ...
    this.supportsConsumption = (sysInfo.feature && sysInfo.feature.includes('ENE') || false);
    this.emitEvents();
  }
  ...
  get (command) {
    return new Promise((resolve, reject) => {
      ...
      socket.on('data', (dataChunk) => {
        ...
        // ignore 4 bytes the first time, 1 byte following times
        var bytesToIgnore = data.length == 0 ? 4 : 1;
        data += decrypt(dataChunk.slice(bytesToIgnore)).toString('ascii');
        try {
          data = JSON.parse(data);
        } catch(ex) {
          // if the data fails to parse, then there's more coming
          return;
        } finally {
          socket.end();
        }
